...
# --- Configuration ---
TOKEN = '8001002276:AAFShcdn4QG-5tF-U2s3D4HjLJ0R1RVMY-I' # Replace with your actual token
OWNER_ID = 8011959413 # Replace with your Owner ID
ADMIN_ID = 8011959413 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@Bruzely' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 't.me/BsbBan' # Replace with your update channel link
...
approved_users = set([8011959413])  # Only admin is approved by default

def is_approved(user_id):
    return str(user_id) in approved_users

def approve_user(user_id):
    approved_users.add(str(user_id))

@bot.message_handler(commands=['approve'])
def handle_approve(message):
    if str(message.from_user.id) != "8011959413":
        bot.reply_to(message, "⛔ You are not authorized to approve users.")
        return
    try:
        parts = message.text.strip().split()
        if len(parts) == 2:
            user_to_approve = parts[1]
            approve_user(user_to_approve)
            bot.reply_to(message, f"✅ Approved user: {user_to_approve}")
        else:
            bot.reply_to(message, "Usage: /approve <user_id>")
    except Exception as e:
        bot.reply_to(message, "Error: " + str(e))

# Global handler to block non-approved users
@bot.message_handler(func=lambda message: not is_approved(message.from_user.id))
def block_unapproved(message):
    bot.reply_to(message, "❌ You are not approved to use this bot. Contact the admin.")
    return
